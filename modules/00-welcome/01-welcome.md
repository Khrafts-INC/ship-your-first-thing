...
This course is **Ship Your First Thing**. It's an open-source, self-paced course for people who have never written production code and want to build a real, deployed product using AI coding tools. By the end, you will have built a small social platform — sign-up, profile, posts, follows, feed, comments, likes — deployed on the internet at a URL anyone can visit.

There are two surfaces:

1. The **course material** — the markdown you're reading right now. This is the canonical source. It lives in a public GitHub repository so anyone can fork it, contribute to it, or read it offline.
2. The **course platform** — a web app at [shipyourfirstthing.com](https://shipyourfirstthing.com) that renders the same lessons with personalized features (sign-in, progress tracking, comments, bookmarks, completion certificate). The platform is built in a separate phase of this project; for now, focus on the markdown.
...
- A deployed social-platform thread project at a URL anyone can visit.
- A working mental model of how software is built — clients, servers, databases, APIs, auth, deployment.
- Real fluency in working with an AI coding agent: planning, executing, recognizing wrong output, recovering.
- A short shelf of habits — git push frequently, run multi-account tests, watch your token budget — that are durable across whichever AI tool you use next.
...
- The free path is real and rate-limited. The paid path is predictable and costs ~$20/month. The "token-careful" path is cheap if you're careful and ruinous if you're not. Module 0 lesson 03 walks you through the triage.
...
3. Open `modules/01-mental-models/README.md`. Read the three bundle titles. This is what Module 1 covers, and it is the most-skipped module in courses like this. (Don't skip it.)
...
- Name the three Module 1 bundles without re-opening Module 1's README.
